...
{
    public class Mfrc522Controller : CardTransceiver, IDisposable
    {
        ...
        public (Status status, byte[] data) AntiCollision()
        {
            ...
            var (status, backData, _) = SendCommand(Command.Transceive, serNum.ToArray());
        ...
        public (Status status, byte Sak) SelectTag(byte[] serialNumber)
        {
            ...
            buf.Add(pOut[0]);
            buf.Add(pOut[1]);
            var (status, backData, backBits) = SendCommand(Command.Transceive, buf.ToArray());
        ...
        public void WriteCardData(byte blockAddress, byte[] writeData)
        {
            ...
            buff.Add(crc[0]);
            buff.Add(crc[1]);
            var (status, backData, backLen) = SendCommand(Command.Transceive, buff.ToArray());
            ...
            (status, backData, backLen) = SendCommand(Command.Transceive, buf.ToArray());
        ...
        public override int Transceive(byte targetNumber, ReadOnlySpan<byte> dataToSend, Span<byte> dataFromCard)
        {
            ...
            else if (dataToSend[0] == (byte)RequestMode.Read)
            {
                ...
                (status, backData, backLen) = SendCommand(Command.Transceive, dataToSend.ToArray());
                if (status != Status.OK)
                {
        ...
        private int TwoStepsIncDecRestore(ReadOnlySpan<byte> dataToSend, Span<byte> dataFromCard)
        {
            ...
            var (status, backData, backLen) = SendCommand(Command.Transceive, dataToSend.ToArray());
            if (status != Status.OK)
            {
